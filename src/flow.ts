...
export class Flow {
    ...
    public bindTelegram(type: string, test: ((data: any) => boolean) | Nil, handler: ((data: any) => void)) {
...
export class BoundList<T> {
    ...
    private isInSync() {
        ...
        for (let index = 0; index < this.__bound.length; index++) {
            ...
            if (!flow || flow._boundValue != goal[index]) return false;
        }
        return true;
    }
...
export class Route {
    ...
    private _onPreLoad?: Nil | ((path: { [key: string]: string }) => Promise<void>);
