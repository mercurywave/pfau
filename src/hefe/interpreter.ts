...
export class Interpreter{
    static __gen = 0;
    public static async Process(input: InputContext, parse: ParseContext, debugLine: number): Promise<TransformResult | null>{
    ...
    public static async ProcessInnerScope(state:InterpreterState, stream: Stream): Promise<boolean> {
        ...
        while(state.statementLine < state.__code.length){
            if((state.nextStatement?.scopeDepth ?? 0) <= curDepth) { return true; }
            state.statementLine++;
            if (!await this.RunOneLine(state)) {return false;}
        }
        throw 'could not process inner scope';
    }
    ...
    public static async RunUserFunction(context: ExecutionContext, func: SFunctionDef, params: Stream[], stream: Stream):Promise<Stream | null>{
        ...
        if(debugLine < state.statementLine) debugLine = 99999999;
        while(state.statementLine < state.__code.length) {
            try{
...
export class ExecutionContext{
    ...
    public saveSideOut(name: string, value: Stream){
...
export class InterpreterState{
    __root: StackBranch;
    __code: IStatement[] = [];
    __scopes: (IStatement | null)[] = [null];
    __debugLine: number;
    __inputContext: InputContext;
    public functionDefs: Record<string, SFunctionDef>;
    public statementLine: number = 0;
    public lastStatement?: IStatement = undefined; // last actually evaluated statement - ignores nulls
    public expectingInnerScope: boolean = false;
    public get currStatement(): IStatement | undefined { return this.__code[this.statementLine]; }
    public get nextStatement(): IStatement | undefined { return this.__code[this.statementLine + 1]; }
    ...
    public exportSideOutputs(): [string, Stream][]{
        let outputs:[string, Stream][] = [];
        this.foreachChain((c,l) => {
...
class StackBranch{
    ...
    public get(name: string): Stream | undefined{
    ...
    public set branches(leafs: StackBranch[] | undefined) {this.__branches = leafs;}
    public get branches(): StackBranch[] | undefined { return this.__branches; }
...
export class VirtualFolder {
    ...
    public async getFileHandle(name: string): Promise<FileSystemFileHandle | undefined>{
        if(this._selectFolder)
