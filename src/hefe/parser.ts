import { ExecutionContext, Interpreter, LineError, CompMatch } from "./interpreter";
import { Lexer, LexLine } from "./lexer";
import { Match, pattern, PatternResult, SingleMatch, Syntax } from "./patterns";
import { IKey, Stream } from "./stream";
...
export class Parser{
    public static Parse(code: string): ParseContext{
        ...
        for (let ln = 0; ln < lines.length; ln++) {
            ...
            try {
                ...
                else throw `could not parse line: ${ln}`
                context.push(state);
            } 
    ...
    public static ParseLine(context: ParseContext, code: string): IStatement | null{
    ...
    public static ParseStatements(context: ParseContext, tokens:string[]): IStatement | null{
        let idx = 0;
    ...
    public static tryParseExpressions(context: ParseContext, tokens: string[]): IExpression[] | null{
    ...
    public static tryParseExpression(context: ParseContext,tokens: string[]): IExpression | undefined{
...
export class ParseContext{
    ...
    public lex!: LexLine;
    private _parseStateFunc?: SFunctionDef = undefined;
    ...
    public prepNewLine(lex: LexLine){
        this.lex = lex;
        if(lex.TabDepth == 0) this._parseStateFunc = undefined;
    }
    ...
    private get currScope(): IStatement | undefined {
        return (this._scopes.length > 0 ? this._scopes[this._scopes.length -1 ] : undefined);
    }
...
export abstract class SScopeFunction extends ICanHaveScope{
    __preExp?: IExpression = undefined;
    __postExp?: IExpression = undefined;
...
function scopeStatementRemainder(): SingleMatch<string>{
...
function scopeStatementPrefix(stop: string): SingleMatch<string> {
    return Match.testPrefix(t => t === stop,
        tokes => {
...
function scopeStatement(func: string): SingleMatch<string>[][]{
    ...
    return [
        [
            ...
            debugToken(),
            token(splitter),
            expressionLike(undefined, false, "post"),
        ],
        [ token(func) ],
        [
...
function expressionLike(stop?: string, optional?: boolean, key?: string):SingleMatch<string> {
    return Match.testSequence(tokes => {
        ...
        if(stop && trail === stop && lPars == rPars) return false;
...
function parameterList(optional?: boolean): SingleMatch<string>{
    return Match.testPattern(pattern(
        token("("),
        expressionLike(undefined, true), // this doesn't seem to actually be optional?
        token(")")
    ), optional, "params");
}
...
function getParamDefListIdents(params: string[]): string[] | null{
...
class SSortBy extends SScopeFunction{
    ...
    public async __onCloseChildScope(context: ExecutionContext, streams: Stream[]): Promise<Stream> {
        const prev = context.scratch.asArray();
        let idxes = Object.keys(prev).map(k => Number(k));
        idxes.sort((a,b) => {
...
class ENumericLiteral extends IExpression{
    ...
    public async Eval(context: ExecutionContext, stream: Stream): Promise<Stream> {
        return Stream.mkNum(this.__num);
    }
}
