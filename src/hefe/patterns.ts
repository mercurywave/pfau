...
export module Match{
    ...
    export function testToken<T>(match: (test:T) => boolean, optional?: boolean, key?: string):SingleMatch<T> {
        return {
            Optional: !!optional,
            Handler: (t,b) =>{
    ...
    export function testPrefix<T>(stopper: (test:T) => boolean, validator: (tokes: T[]) => boolean | null, optional?: boolean, key?: string):SingleMatch<T>{
        return {
            Optional: !!optional,
            Handler: (t,b) =>{
                let slice = t.slice(b);
    ...
    export function testPattern<T>(pattern: Pattern<T>, optional?: boolean, key?: string):SingleMatch<T>{
        return {
            ...
            Handler: (t,b) =>{
                ...
                return result(!!res?.isSuccess, res?.startIndex ?? 0, res?.length ?? 0, key ?? "patt");
            },
        };
    }
}
...
export class Pattern<T>{
    ...
    public testWhole(tokens: T[], fromIdx: number): PatternResult<T> | null {
        ...
        if((res?.length ?? 0) + fromIdx < tokens.length) { return null; }
        return res;
    }
}
...
export class Syntax<T,V>{
    ...
    public testPartial(tokens: T[], fromIdx: number):V | null{
        ...
        return this.maps[result.index]?.output ?? null;
    }

    public firstPartialMatch(tokens: T[], fromIdx: number): SyntaxMatch<T, V> | null{
