...
regFunc("piece", 2, 2, ["delim", "pieceNum"], async (c, stream, pars) =>{
    ...
    if(idx > split.length) return Stream.mkText("");
...
regFunc("push", 1, 1, ["value"], async (c, stream, pars) =>{
    const array = stream.asArray();
    const val = (await pars[0]!.Eval(c, c.stream));
    return Stream.mkArr(array.concat(val));
});

regFunc("concat", 1, 1, ["values"], async (c, stream, pars) =>{
    const array = stream.asArray();
    const values = (await pars[0]!.Eval(c, c.stream)).asArray();
    return Stream.mkArr(array.concat(...values));
});
...
regFunc("reReplaceAll", 1, 3, ["regExp"], async (c, stream, pars) =>{
    ...
    let caseInsensitve = false;
    let repl = "";
    if(pars.length > 1) repl = (await pars[1]!.Eval(c, stream)).asString();
    if(pars.length > 2) caseInsensitve = (await pars[2]!.Eval(c, stream)).asBool();
    try{
        const regex = new RegExp(code, "g" + (caseInsensitve ? 'i' : ''));
...
regFunc("sideOutput", 0, 1, ["name"], async (c, stream, pars) =>{
    let name ="";
    if(pars.length > 0) name = (await pars[0]!.Eval(c, stream)).asString();
    try{
        const invalidChars = /[\\\/:*?"<>|\n\r\t]/; // disallow common problem characters
        if(invalidChars.test(name)) throw new Error(`${name} is not a valid file name`);
        c.saveSideOut(name, stream);
        return stream;
    } catch(e){ throw new Error(`sideOutput: ${e}`)}
});
